...
public class SubdomainVisitCount {
    public List<String> subdomainVisits(String[] cpdomains) {
        ...
        for (String cd : cpdomains) {
            String[] hitAndDomain = cd.split(" ");
            int hit = Integer.parseInt(hitAndDomain[0]);
            String[] cds = hitAndDomain[1].split("\\.");
            sb.delete(0, sb.length());

            for (int i = cds.length - 1; i >= 0; --i) {
                sb.insert(0, sb.length() == 0 ? cds[i] : cds[i] + ".");

                if (!visits.containsKey(sb.toString())) {
                    visits.put(sb.toString(), 0);
                }

                visits.put(sb.toString(), visits.get(sb.toString()) + hit);
            }
        }
